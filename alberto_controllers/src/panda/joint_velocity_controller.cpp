...
controller_interface::InterfaceConfiguration JointVelocityController::state_interface_configuration() const
{
  ...
  for (std::size_t i = 1; i <= NUM_JOINTS; ++i)
  {
    const std::string joint_name = arm_id_ + "_joint" + std::to_string(i);
...
controller_interface::return_type JointVelocityController::update( const rclcpp::Time&, const rclcpp::Duration&)
{
  if (command_interfaces_.size() != NUM_JOINTS)
  {
    RCLCPP_ERROR_THROTTLE( get_node()->get_logger(), *get_node()->get_clock(), 1000,
      "Expected %zu command interfaces, found %zu", NUM_JOINTS, command_interfaces_.size());
...
CallbackReturn JointVelocityController::on_init()
{
  ...
  catch (const std::exception& exception)
  {
    RCLCPP_ERROR( get_node()->get_logger(),
      "Initialization failed: %s", exception.what());
...
{
  command_.fill(0.0);
  last_msg_time_ = get_node()->now();

  joint_velocity_subscriber_ = get_node()->create_subscription<std_msgs::msg::Float64MultiArray>(
        command_topic_,1,
        std::bind(
          &JointVelocityController::jointVelocityCommandCallback,
          this,std::placeholders::_1));

  RCLCPP_INFO(get_node()->get_logger(), "Activated joint velocity controller for '%s' on '%s'", arm_id_.c_str(), command_topic_.c_str());

  return CallbackReturn::SUCCESS;
}

CallbackReturn JointVelocityController::on_deactivate( const rclcpp_lifecycle::State&)
{
  command_.fill(0.0);
...
CallbackReturn JointVelocityController::on_error( const rclcpp_lifecycle::State&)
{
  command_.fill(0.0);
...
void JointVelocityController::jointVelocityCommandCallback( const std_msgs::msg::Float64MultiArray& msg)
{
  if (msg.data.size() != NUM_JOINTS)
  {
    RCLCPP_ERROR( get_node()->get_logger(),
      "Expected %zu joint velocities, received %zu", NUM_JOINTS, msg.data.size());
